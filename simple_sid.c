...
/* ------------------------------------------------------------------
   (longer) helper: noise
   ------------------------------------------------------------------ */
static unsigned noiseSidChannel(sidChannel_t *ch)
{
    unsigned tmp = 0;
...
/* ------------------------------------------------------------------
   ...
   ------------------------------------------------------------------ */
static void clockSidChannel(sidChannel_t *ch, int cycles)
{
        ...
        while (left > 0) {
            ...
            if (ch->waveform & 0x80) {
                ...
                if (mask20 < 0x80000) {
                    ...
                    if (needed < stepNow) stepNow = needed;
                } else {
            ...
            if (ch->syncTarget->waveform & 0x02) {
                if (ch->accumulator < 0x800000) {
                    int needed = (int)((0x800000 - ch->accumulator)
                                        / ch->frequency) + 1;
                    if (needed < stepNow) stepNow = needed;
                } else {
                    int needed = (int)((0x1800000 - ch->accumulator)
                                        / ch->frequency) + 1;
                    if (needed < stepNow) stepNow = needed;
                }
            }
            ...
            ch->accumulator = (ch->accumulator +
                              (ch->frequency * (unsigned)stepNow)) & 0xffffff;
...
/* ------------------------------------------------------------------
   ...
   ------------------------------------------------------------------ */
static float getOutputSidChannel(sidChannel_t *ch)
{
    if (ch->volumeLevel == 0)
        return 0.f;
    ...
    case 0x50: /* Tri + Pulse */
        {
            ...
            waveOut = combo << 1;
            if (waveOut > 0xffff) waveOut = 0xffff;
        }
        break;
                         ...
                         int32_t     maxSamples)
{
    ...
    float cutoff = 0.05f + 0.85f * (
        sinf(((float)regs->cutoff / 255.f - 0.5f) * (float)M_PI)*0.5f + 0.5f
    );
    ...
    while (cpuCycles > 0 && outIndex < maxSamples) {
        ...
        if (sid->cycleAccumulator >= sid->cyclesPerSample) {
            ...
            /* channel0 -> filter or direct? */
            {
                ...
                if (filterCtrl & 0x01) fin += c0;
                else                   out += c0;
            }
            /* channel1 */
            {
                float c1 = getOutputSidChannel(&sid->channels[1]);
                if (filterCtrl & 0x02) fin += c1;
                else                   out += c1;
            }
            /* channel2 */
            {
                float c2 = getOutputSidChannel(&sid->channels[2]);
                if (filterCtrl & 0x04) fin += c2;
                else                   out += c2;
            }

            /* Filter the mixed channels */
            float filtered;
            sidFilterStep(fin, cutoff / 2047.0f, resonance / 128.0, regs->filterCtrl, &sid->filter, &filtered);
            out += filtered;
